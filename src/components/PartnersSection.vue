      ...
      <div v-else class="relative w-full overflow-hidden py-4">
        <div class="animate-marquee-partners">
          <div v-for="n in 2" :key="n" class="flex gap-4 shrink-0">
            ...
            >
            <div 
             class="w-12 h-12 sm:w-17 sm:h-17 rounded-2xl flex items-center justify-center text-base sm:text-lg font-bold transition-colors bg-slate-200 text-slate-600 group-hover:bg-[#26d0ce]/20 group-hover:text-[#26d0ce] shrink-0 overflow-hidden"
                  >
                     <img 
                      v-if="partner.logo" 
                      :src="partner.logo" 
                      alt="Partner Logo" 
                        class="w-full h-full object-contain p-1"
                          /> 
                        <span v-else class="text-sm">{{ getInitials(partner.name_i18n?.[locale] || partner.name) }}</span>
                           </div>
